...
//함수의 파라미터로 배열주소를 포인터 방식 표현 예시 (int p[] , int *p 중 int p[] 선택)
...
void input(int *p, int cnt) {           // int p[] == int *p;
    ...
    // 배열크기만큼 for문을 돌지만, 로직상 포인터변수  p의 메모리 값을 4byte씩 배열크기만큼 반복하는 것일 뿐이다.
    for (int i = 0; i < cnt; i++) {

        printf("생성 %3d : ", i + 1);
        *p = rand() % rand_range + 1;        // p[i] = *(p+i)
        printf(" %4d\n", p++);                      // 이 코드 이후 후위연산자로 p = p + 1로 업뎃되어 a의 주소는 계속 한 칸씩 뒤로 간다
    }
}
