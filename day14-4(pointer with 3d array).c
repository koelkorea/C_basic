...
//                  -> (중요!) 컴파일러에 따라 단독으로 쓴 '3차원배열변수[]'는 진짜 3차원배열변수[][0][0]와 어떤 연관관계도 찾을 수 없는 의미없는 값인 경우가 존재함
...
//                 [예시1] *(pScore2 + i) != *(score_3D[2] + i)  -> *(pScore2 + i) = *(*(score_3D[2] + i))
...
int main() {
    ...
    // pScore1 = score_3D = &score_3D[0][0][0]       <- score_3D 단독은 2중 포인터변수로 score_3D[0][0][0]의 주소값과 아무런 연관이 없음
    int* pScore1 = score_3D;                         // 포인터변수에 대입했을 경우 score_3D[2] = &score_3D[2][0][0]로 인식... 

    // pScore2 = score_3D[2] = &score_3D[2][0][0]    <- score_3D[2] 단독은 2중 포인터변수로 *score_3D[2] = &score_3D[2][0][0] , **score_3D[2] = score_3D[2][0][0]
    ...
    for (int i = 0; i < 5; i++) {
        ...
        // 정황상 C언어는 score_3D를 score_3D[0][0][0]와 아무런 연관이 없는 주소값으로 판정한거 같음
