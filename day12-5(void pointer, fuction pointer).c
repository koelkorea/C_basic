...
// 함수 포인터(* pFunction )
//  : 함수가 위치한 메모리의 주소값을 저장할 수 있는 포인터로 사실상 해당 함수와 동의어로 사용 가능!
   
//   # 함수 포인터 선언법 ( asterisk(*) 연산자 사용)
//      : 자료형 (* 함수포인터변수명) (매개변수1, 매개변수2, …)
//          -> 이후.. 함수포인터변수명(파라미터1, 파라미터2...) = 함수명(파라미터1, 파라미터2...)와 같은 의미를 가짐
...
//   # 함수 포인터 특성
//      1.(중요) 함수포인터명은 메모리의 text segment 영역에 위치한 시작 주소 값을 의미 
//         -> 그 주소값을 기반으로 stack에 함수프레임을 만들어서 지역변수와 return값을 할당할 주소를 할당함)
// 
//      2. (중요) 함수 포인터의 자료형, 매개변수는 받는 함수의 반환형과 매개변수를 의미하므로 반드시 맞춰야 함
//      3. 함수포인터도 역시 포인터이므로, 초기화 해줘야함 (NULL이나 0을 추천)
