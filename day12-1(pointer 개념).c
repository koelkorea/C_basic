...
//			ex) int* score;			<- int 자료형의 주소를 저장하는 포인터 변수인 score를 선언 (int * 의미 : 변수 a 시작 주소 + sizeof(int) 까지 )
...
//		# 역참조 형식 : *(선언된) 포인터변수명				<-  포인터의 시작 주소 + 자료형 크기로 메모리 읽기 

//			ex) int * score = &a;           <- 포인터변수 *score 주소 : 0x11111, *score 값 : 0x222222 (변수 a의 주소)			
