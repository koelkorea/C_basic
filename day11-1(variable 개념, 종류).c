...
//  - {} 를 기준으로 따지는 변수의 종류
//   1. 전역변수
//       : 함수 밖이나 외부파일에서 선언 = 프로그램 전체에 걸쳐 사용될 수 있는 변수
//          -> 자동 초기화 ㅇ (숫자는 0)  
//          -> 생존기간 : forever  = 데이터 영역에 메모리(RSS)가 할당
//          -> 확장개념 : 외부 static함수, 외부변수(extern) 

//   2. 지역변수
//       : 선언된 블록이나 함수같은 특정 범위({} 스코프) 내에서만 통용되는 변수
//          -> 자동 초기화 X
...
//   1. 자동변수(auto)
//       : 함수 스코프 안에서 선언되고, 그 함수 안에서만 사용되며, 함수가 종료되면 stack메모리에서 해제 (일종의 지역변수)
//          -> 자동 초기화 X (지역변수이기에)
//          -> 함수가 실행될 때마다, 새로 초기화 된다는 점 명심! 
//          -> auto라는 예약어는 생략가능
//
//   2. 정적 변수(static)
//       : 컴파일 시 데이터 영역에 메모리(RSS)가 할당, 프로그램이 종료될때까지 값을 유지 (단! 자기 프로그램 내에서만 참조 가능)
...
//
//   3. 외부변수(extern)
//       : extern으로 선언한 전역변수는 함수의 외부에 선언되는 일종의 전역변수 = 다른 파일에서 해당 값을 참조 가능 (전역변수, 일반 정적변수는 이게 불가능) 
...
//
//  4. 레지스터변수(register)
//      : 지역변수지만, CPU의 레지스터에 저장되는 지역변수 (CPU에 저장된다는거 말고는 지역변수와 같음)
...
// 3. 외부변수(extern)
//     : ex_a ~ f는 외부파일에 선언된 녀석들을 참고 (day11-3.c에 선언)
...
void check() {
    ...
    // 2.2 내부 static 변수
    //      : check() 함수 안에서 선언된 static변수 (외부 static이나, 다른 스코프 안의 내부 static과는 상관없음)
    static int C = 10;
...
int main() {
    ...
    // 2.2 내부 static 변수
    //      :  main() 함수 안에서 선언된 static변수 ( check()의 변수 C와는 상관없음... 다른 차원의 C임)
    static int C = 40;
    ...
    if (i > 0) {
        ...
        // (중요)  반복문 차원에 선언된 지역변수 k는 본차원과 관계X  이므로... 본차원의 k는 선언되기 전까지 존재 X (js 초기버전의 var가 이 스코프 처리가 개판이라.. 문제)
        for (int k = 0; k < 10; k++) {  
            printf("k = %d , value = %d\n", k, value1++);
        }
