...
//  - 자료형 (*를 역참조할 차원수만큼 반복 X중포인트배열명)[참조하는 고차원배열 1차원크기].....[참조하는 고차원배열 i차원크기] = 해당 고차원 배열명;
...
//          int score[5][5][5];
//          int** tmp = score[0];   <- tmp + i => &score[i][0][0]   (= 아시다시피 행단위로 의도하대로 움직일거면 꼭 치환해줘야 함)
//          int pScore2[5] = {(**tmp), (**(tmp + 1)), (**(tmp + 2)), (**(tmp + 3)), (**(tmp + 4))}
//                         = {(**score[0]), (**(score[0] + 1)), (**(score[0] + 2)), (**(score[0] + 3)), (**(score[0] + 4))}
//                         = {(**score[0]), (**(score[1])), (**(score[2])), (**(score[3])), (**(score[4]))}

//          -> 1. 2중 포인터변수 tmp가 내부적으로 선언되고, 그 값은 score의 1차원 행을 주소값을 시작으로 대입된다 (= pScore2의 주소값 연산은 1차원 단위로 움직임)
...
int main() {
    ...
    int (*pScore2)[5] = score;
    //  int pScore2[5] = {(*score[0]), (*(score[1])), (*(score[2])), (*(score[3])), (*(score[4]))}
