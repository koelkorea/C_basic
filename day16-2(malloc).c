...
//      ex) 동적할당으로 int score1[n] 선언하는 효과 내기 (= 동적할당 함수의 리턴값인 시작주소값이 int의 포인터변수로 해석되도록 형변환 시킴으로서 각 배열의 index처럼 순차적인 주소의 값에 접근)
...
//      ex) 동적할당으로 int* score2[n] 선언하는 효과 내기(= 동적할당 함수의 리턴값인 시작주소값이 int*의 포인터변수(int의 포인터변수의 위치를 나타내는 2중포인터 변수)로 해석되도록 형변환 시킴으로서 각 포인터 배열의 index처럼 순차적인 주소에 위치한 int변수의 주소값에 접근)
