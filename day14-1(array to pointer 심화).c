...
//          - []와 함께 경우
//             1. 2차원배열변수[] : 1차원에 해당하는 부분을 의미하는 변수의 표기값 그 자체가 주소를 의미하는 '포인터변수'(= &2차원배열변수[1][0] = 2차원배열변수[2])
//             2. 2차원배열변수[][] : 배열 안에 위치하는 단순 자료형 변수 (= 포인터와 관련성이 1도 없음)

//        -> n차원 배열도 메모리 할당은 선형성을 띄는 연속적 할당이기 때문

//           ex) 포인터식 표현      배열식 표현   : 2차원 배열로 선언된 배열의 포인터의 경우 1차원까지를 표기한 경우는 그 
//               *(p[3] + 3)   =     p[3][3]
