...
//      - 공통점 : 원시형태의 객체형식
//      - 차이점 : 구조체는 모든 멤버변수의 메모리를 전부 할당      <->     공용체는 멤버변수 중 가장 큰 메모리를 가진 자료형의 변수 1개에만 할당 (집단은 존재하나, 사실상 나머지는 존재하지 않는 유령회사라 생각하자)
...
int main() {
    ...
    printf("Union 정수 (손실): %10d \t \t union    int size : %3d\n", u_num.i, (int)sizeof(u_num.i));               // (중요) int 와 double을 가진 경우, double이 입력되도 이전에 있던 int값에는 접근이 가능하나.. 그 값은 주어진 8byte에 덮어씌워진 double 중 4byte를 int로 읽은 값에 불과함
