...
int main() {
	...
	// [증감연산자]
	//  : +- 1을 연산하되, 전위 연산자는 전위연산자 턴에 일괄적인 변수 계산을 마친 값을 최종결과값으로 해당 변수에 적용하고, 
	//                     후위는 해당 코드의 모든 연산이 끝난 후 일괄적으로 변수 계산을 마친 값을 최종결과값으로 해당 변수로 적용
	// 
	//   - 전위연산자 (++a, --a)
	//      : 변수값 a에 1이 더하고 빼짐 
	//        (단, 해당 코드의 모든 전위 연산자의 계산은 순차적인게 아니라, 전위턴에 한번에 일괄계산한 값을 적용한다)
	// 
	//   - 후위연산자 (a++, a--)
	//      : 변수값 a에 당장 1이 더하고 빼지지 않고, 연산이 실행되는건 해당 코드의 모든 연산이 끝나고 난 뒤 수행함 
	//        (표기되는 연산값이 오리지널 a)
	//         -> 참고로 중첩해봐야 1번 뿐이 적용이 안됨
	// 
