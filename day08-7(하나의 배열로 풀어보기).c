...
int main() {
	...
	// 짝수는 선택정렬
	int  min = 0, address = 0, even = 0, odd = 0;		// 최대값과 최소값을 저장할 주소 추가 + 홀수 / 짝수 경계 주소 저장값
	...
	for (int i = 0; i <= 9; i++) {
		...
		if (temp % 2 == 0) {	// 짝수 일 경우 99-(짝수의 현재개수)에 해당하는 배열주소에 값 저장 + 짝수의 현재개수 + 1
			array[9 - even] = temp;
			even += 1;
		}
		else if (temp % 2 == 1) {	// 홀수인 경우 i - (짝수의 현재개수) 에 해당하는 배열주소에 값 저장
			array[i - even] = temp;
		}
	}
