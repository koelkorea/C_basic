...
int main() {
    ...
    // 배열로 선언된 변수명(= 상수 포인터)
    //  : 그 자체가 해당 배열 0번방(1번째방)의 주소를 의미 (&이 필요없는 이유)
    ...
    // | c[0] = 1.50 | *fp = 1.50 | *fp + 2 = 3.50 | *(fp + 2) = 9.50 |
    ...
    // | *p + 2 = 5 | *(p + 2) = 7 | *q - 2 = 7 | *(q - 2) = 5 |
    printf("| *p + 2 = %d | *(p + 2) = %d | *q - 2 =%d | *(q - 2) = %d |\n\n", *p + 2, *(p + 2), *q - 2, *(q - 2));

    // | *(p - 3) = -858993460 | *(q + 13) = 2 |
    printf("| *(p - 3) = %d | *(q + 13) = %d |\n\n", *(p - 3), *(q + 13));     
    ...
    //  -> BUT! 같은 int 배열인 b의 값들이 위치하는 곳까지 주소가 도달한다면.. 경우에 따라서는 참조가 가능하다는 것을 확인 가능
    for (int i = 0; i < 25; i++) {
        printf("  p+i 값 : %d \n", *(p + i) );
    }
    ...
    // add = p + q;             <- 포인터 + 포인터 연산이라 불가능!
    // p = fp;                  <- 포인터 간 자료형이 다름 : 연산 불가 or 역참조 불가
