...
//     : 자료형* 포인터변수명[원소의 수]    <- 배열 내부에 특정 자료형의 포인터변수들을 저장할 수 있는 포인터 배열 선언

//      ex1) int* a[3];                     <- (중요) 배열크기가 3인 int형 포인트변수 배열a를 선언' 함을 의미 
//           int b = 1, c = 2, d = 3      
//           int* a[3] = { &b, &c, &d};     <- 배열 안에 요소로 int 포인터변수 입력 가능 
...
//          3) pointer_array1[1]는 int 포인터변수 배열의 2번째 값 = 2차원 배열 array_2d[1][0]의 주소값을 의미 
//              ->   pointer_array1[1]  ==   array_2d[1]  == &array_2d[1][0]
//              -> *(pointer_array1[1]) == *(array_2d[1]) ==  array_2d[1][0]

//          4) double_pin_point는 int 포인터변수 배열의 시작 주소의 위치값을 의미 
