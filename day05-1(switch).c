...
//         2. case별 스코프 {}가 없고 switch문당 하나 뿐이라... 
//            (= if문과 다르게, 지역변수가 케이스별로 구분되기 쉽지 않다..)
...
int main() {
	...
	switch (c) {		
		...
		// case 케이스명 : 작동코드'로 경우의 수에 따른 코딩 가능, 단 break문을 써줘야 그 조건에서 조건문이 종료 
		//                 (= 안 끊으면 그 밑의 경우의 수 코드도 다 작동..)

		// break         : 여기서 조건문, 반복문을 끝내고 빠져나오라는 코드 
		//                 (= 정확히는 스코프{}를 빠져나오라는 말)
		
		// default       : 조건문의 else에 해당하는 녀석
